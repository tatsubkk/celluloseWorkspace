...
def get_psi_along_streamline(
    ...
    if model == "C1":
        ...
        t_extrap_tol = 1e-3
        t_data_min, t_data_max = float(np.min(t_data)), float(np.max(t_data))

        if t_end > t_data_max + t_extrap_tol:
            raise ValueError(
                f"t_end is too far outside gradu interpolation range: "
                f"t_end={t_end}, data range=({t_data_min}, {t_data_max}), "
                f"tol={t_extrap_tol}"
                )
        ...
        # p
        p = np.zeros((Np*Nt, 3), dtype=float)
        p[:, 0] = sinTHETA * cosPHI         # x
        p[:, 1] = sinTHETA * sinPHI         # y
        p[:, 2] = cosTHETA                  # z

        # ePHI
        ePHI = np.zeros((Np*Nt, 3), dtype=float)
        ePHI[:, 0] = -sinPHI                # x
        ePHI[:, 1] = cosPHI                 # y
        ePHI[:, 2] = 0.0                    # z

        # eTHETA
        eTHETA = np.zeros((Np*Nt, 3), dtype=float)
        eTHETA[:, 0] = cosTHETA * cosPHI    # x
        eTHETA[:, 1] = cosTHETA * sinPHI    # y
        eTHETA[:, 2] = -sinTHETA            # z
        ...
        jac_sparsity = jac_sparsity.astype(bool)
        ...
        if set_initial_state == True:
            ...
            from cnfkit.simulation_20260511.psi_equilibrium import get_psi_equilibrium
            Psi0 = get_psi_equilibrium(
                gradu=gradu_interp(t_start),
                r=r,
                Dr=Dr,
                Np=Np,
                Nt=Nt,
            ...
            # isotropic
            Psi0 = np.full(Np*Nt, 1.0/(2.0*np.pi), dtype=float)
        ...
        try:
            ...
            raise KeyError(f"C2_r requires parameter '{e.args[0]}' in model_params") from None
        ...
        try:
            ...
            raise KeyError(f"C2_iso requires parameter '{e.args[0]}' in model_params") from None
        ...
        try:
            ...
            raise KeyError(f"C2_r_iso requires parameter '{e.args[0]}' in model_params") from None
        ...
        try:
            r = model_params["r"]
            Dr_fast = model_params["Dr_fast"]
            Dr_middle = model_params["Dr_middle"]
            Dr_slow = model_params["Dr_slow"]
            alpha_fast = model_params["alpha_fast"]
            alpha_middle = model_params["alpha_middle"]
        except KeyError as e:
        ...
        psi_targets_middle, _ = get_psi_along_streamline(
            gradu_track_path = gradu_track_path,
            gradu_track_idx = gradu_track_idx,
            model = "C1",
            model_params = {"r":r, "Dr":Dr_middle},
            x_targets = x_targets,
            Np = Np,
            Nt = Nt,
            set_initial_state=set_initial_state,
        ...
        psi_targets = (
            alpha_fast * psi_targets_fast
            + alpha_middle * psi_targets_middle
            + (1.0 - alpha_fast - alpha_middle) * psi_targets_slow
        )

        return psi_targets, psi_info
    
    else:
        valid_models = ["C1", "C2", "C2_r", "C2_iso", "C2_r_iso", "C3"]
        raise ValueError(
